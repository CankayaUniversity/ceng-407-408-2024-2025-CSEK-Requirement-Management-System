...
class Subsystem1RequirementsController{
  ...
  static void showAddRequirementDialog(BuildContext context, WidgetRef ref, String username) {
    ...
    final systemRequirements = ref.read(systemRequirementListProvider).value ?? [];
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<SystemReqModel>(
                isExpanded: true,
                decoration: const InputDecoration(labelText: "Sistem Gereksinimi Seç"),
                items: systemRequirements.map((req) {
        ...
        actions: [
          TextButton(
            onPressed: () async {
              ...
              await Subsystem1RequirementApiService().createRequirement(newRequirement);
  ...
  static void showDescriptionDialog(BuildContext context, WidgetRef ref, Subsystem1ReqModel req, bool canEdit, String username,
  ...
  static Future<void> createChangeLogForRequirement({
    ...
    final relatedAttributes = attributes.where((a) => a.subsystem1Id == req.id).toList();
        ...
        actions: [
          TextButton(
            onPressed: () async {
              ...
              final newHeader = Header_Sub1Req_Model(
                header: headerText,
              );
          ...
          TextButton(
            onPressed: () async {
              ...
              if (attributeId != null && attributeId.isNotEmpty) {
                ...
                await Sub1AttributeApiService().updateRequirement(updatedAttr);
              } else {
                ...
                await Sub1AttributeApiService().createRequirement(newAttr);
              }

              // Requirement'ı flag=false yap
              final allRequirements = ref.read(subsystem1RequirementListProvider).value ?? [];
              final target = allRequirements.firstWhere(
                    (r) => r.id == subSystemRequirementId,
                orElse: () => Subsystem1ReqModel(id: '', title: '', description: '', createdBy: '', flag: true, systemRequirementId: '', ),
              );
              ...
              if (target.id.isNotEmpty) {
                ...
                final allHeaders = ref.read(sub1HeaderListProvider).value ?? [];
                final allAttributes = ref.read(sub1AttributeListProvider).value ?? [];

                await controller.Subsystem1RequirementsController.createChangeLogForRequirement(
                  username: username,
                  req: target,
                  headers: allHeaders,
            ...
            TextButton(
              onPressed: () async {
                ...
                if (confirmed == true) {
                  ...
                  await Sub1AttributeApiService().deleteRequirement(attributeId!);
                  ref.refresh(sub1AttributeListProvider);
                  Navigator.of(context).pop();
                }
              },
