          ...
          TextButton(
            onPressed: () async {
              ...
              if (attributeId != null && attributeId.isNotEmpty) {
                ...
                await UserAttributeApiService().updateRequirement(updatedAttr);
              } else {
              ...
              final allRequirements = ref.read(userRequirementListProvider).value ?? [];
              final target = allRequirements.firstWhere(
                    (r) => r.id == userRequirementId,
                orElse: () => UserReqModel(id: '', title: '', description: '', createdBy: '', flag: true),
              );
              ...
              if (target.id.isNotEmpty) {
                ...
                final allHeaders = ref.read(headerUserReqModelListProvider).value ?? [];
                final allAttributes = ref.read(userAttributeListProvider).value ?? [];
            ...
            TextButton(
              onPressed: () async {
                ...
                if (confirmed == true) {
                  ...
                  await UserAttributeApiService().deleteRequirement(attributeId);
                  ref.refresh(userAttributeListProvider);
                  Navigator.of(context).pop();
                }
              },
