...
class _ChangeLogPageState extends ConsumerState<ChangeLogPage> {
  ...
  void didChangeDependencies() {
    ...
    Future.microtask(() => ref.invalidate(userRequirementChangeLogListProvider));
    Future.microtask(() => ref.invalidate(systemRequirementChangeLogListProvider));
    Future.microtask(() => ref.invalidate(subsystem1RequirementChangeLogListProvider));
    Future.microtask(() => ref.invalidate(subsystem2RequirementChangeLogListProvider));
    Future.microtask(() => ref.invalidate(subsystem3RequirementChangeLogListProvider));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final changeLogUserReqAsync = ref.watch(userRequirementChangeLogListProvider);
    final changeLogSystemReqAsync = ref.watch(systemRequirementChangeLogListProvider);
    final changeLogSub1ReqAsync = ref.watch(subsystem1RequirementChangeLogListProvider);
    final changeLogSub2ReqAsync = ref.watch(subsystem2RequirementChangeLogListProvider);
    final changeLogSub3ReqAsync = ref.watch(subsystem3RequirementChangeLogListProvider);
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText: selectedPrefix == null ? "Önce tip seçin" : null,
                        ),
                    ...
                    DropdownButton<String>(
                      ...
                      onChanged: (value) => setState(() => selectedChangeType = value),
                      items: ['update', 'delete', 'update-attribute', 'delete-attribute']
                          .map((type) => DropdownMenuItem(value: type, child: Text(type)))
                          .toList(),
                    ),
                    ...
                    DropdownButton<String>(
                      ...
                      onChanged: (value) => setState(() => selectedModifiedBy = value),
                      items: allLogs
                          .map((e) => e.modifiedBy ?? "")
                          .toSet()
                    ...
                    ElevatedButton(
                      ...
                      child: Text("Başlangıç: ${startDate != null ? startDate!.toLocal().toString().split(' ')[0] : 'Seç'}"),
                    ),
                    ...
                    ElevatedButton(
                      ...
                      child: Text("Bitiş: ${endDate != null ? endDate!.toLocal().toString().split(' ')[0] : 'Seç'}"),
                    ),
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              padding: const EdgeInsets.all(12),
                              child: Text(
                                log.modifiedAt?.toLocal().toString().split('.')[0] ?? '-',
                              ),
                            ),
                            ...
                            ...List.generate(log.header?.length ?? 0, (i) {
                              ...
                              final attributeText = (log.oldAttributeDescription?.length ?? 0) > i
                              ...
                              return Container(
                                ...
                                child: Tooltip(
                                  message: '$headerText',
                                  child: Text(
                                    attributeText.length > 20 ? '${attributeText.substring(0, 20)}...' : attributeText,
                                    overflow: TextOverflow.ellipsis,
                                  ),
                                ),
                              );
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Değişiklik Tipi: ${log.changeType?.toUpperCase() ?? '-'}"),
              Text("Yapan: ${log.modifiedBy ?? '-'}"),
              Text("Tarih: ${log.modifiedAt?.toLocal().toString().split('.')[0] ?? '-'}"),
              ...
              if ((log.header?.isNotEmpty ?? false) && (log.oldAttributeDescription?.isNotEmpty ?? false))
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: List.generate(log.header!.length, (i) {
                    final h = log.header?[i] ?? '-';
                    final desc = (log.oldAttributeDescription?.length ?? 0) > i
                        ? log.oldAttributeDescription![i]
                        : '-';
                    return Padding(
