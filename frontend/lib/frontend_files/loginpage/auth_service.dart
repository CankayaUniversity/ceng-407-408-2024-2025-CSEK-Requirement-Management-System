...
class AuthService {
  ...
  static Future<void> loginWithKeycloak() async {
   // print('[AuthService] loginWithKeycloak çağırıldı.');
    ...
    final issuer = await Issuer.discover(Uri.parse('http://localhost:8081/realms/myrealm1'));
    final client = Client(issuer, 'flutter-web');
  ...
  static Future<List<String>> getUserRoles() async {
    ...
    final roles = (decoded['resource_access']?['spring_client']?['roles'] as List?)?.cast<String>() ?? [];
  ...
  static void logout() {
    final redirectUri = Uri.parse('${window.location.origin}/#/login');
    final logoutUrl =
        'http://localhost:8081/realms/myrealm1/protocol/openid-connect/logout'
