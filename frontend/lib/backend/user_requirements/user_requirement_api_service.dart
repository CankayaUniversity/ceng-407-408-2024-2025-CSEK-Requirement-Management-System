...
class UserRequirementApiService {
  final Dio dio = Dio(
    BaseOptions(baseUrl: 'http://localhost:9500/user-requirements'),
  );

  Future<List<UserReqModel>> getRequirements(String projectId) async {
    final response = await dio.get('/user-requirements/$projectId');
    final list =
        (response.data as List).map((e) => UserReqModel.fromJson(e)).toList();
